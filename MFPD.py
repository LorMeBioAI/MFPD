...
parser.add_argument('--identity', type=float, default=0.97,
                    help='Recommended identity: 0.99 for Full ITS AND 0.97 for ITS1/2 , default=0.97')
...
abs_clusters = os.path.join(abs_path, "clusters")
...
if args.region == 'subregion':
    ...
    blastn -query asv_rep.fasta -db {abs_db}/MFPD -outfmt 6 -evalue 1e-5 -perc_identity {identity} -num_threads 6 -out blastnM8.txt
...
"""
    ...
    awk '{{print "{abs_clusters}/cluster_"$1".txt"}}' ./temp/mmseq_search_nodes.txt | xargs cat > ./temp/mmseq_search_subset.txt
    ...
    blastn -query asv_rep.fasta -db ./temp/mmseq_search_subset -out blastnM8.txt -outfmt 6 -max_target_seqs 3 -evalue 1e-10 -perc_identity {identity} -num_threads 32
    perl {abs_bin}/blast_best.pl blastnM8.txt > blastn.txt
    perl {abs_bin}/taxonomy_lenthre.pl blastn.txt {abs_db}/MFPD.ITS.tax asv_rep.temp.tax
    perl {abs_bin}/tax_normal.pl asv_rep.fasta asv_rep.temp.tax asv_rep.tax  
    perl {abs_bin}/taxaTable_byAss.pl asv_table.xls asv_rep.tax asv_taxa_table.xls 
